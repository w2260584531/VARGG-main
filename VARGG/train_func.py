...
class train():
    ...
    def pretrain(
        ...
        with tqdm(total=int(self.pre_epochs), 
                    desc="VARGG trains an initial model",
    ...
    def fit(self, 
        ...
        if clusterType == 'KMeans':
            ...
            cluster_centers_ = np.asarray(Mergefeature.groupby("Group").mean())   #sgc_what
            if self.domains is None:
        ...
        elif clusterType=='BGMM':
            bgmm = BayesianGaussianMixture(n_components=cluster_n, covariance_type='full', max_iter=100, random_state=273)
            bgmm.fit(pre_z)
            y_pred_last = bgmm.predict(pre_z)
            n_clusters = len(np.unique(y_pred_last))
            features = pd.DataFrame(pre_z, index=np.arange(0, pre_z.shape[0]))
            group = pd.Series(y_pred_last, index=np.arange(0, pre_z.shape[0]), name="Group")
            merged_features = pd.concat([features, group], axis=1)
            cluster_centers = np.asarray(merged_features.groupby("Group").mean())
            device=self.device
            if self.domains is None:
                self.model.cluster_layer.data = torch.tensor(cluster_centers).to(device)
            else:
                self.model.model.cluster_layer.data = torch.tensor(cluster_centers_).to(self.device)
            
        with tqdm(total=int(self.pre_epochs), 
                    desc="VARGG trains a final model",
                        bar_format="{l_bar}{bar} [ time left: {remaining} ]") as pbar:
            for epoch in range(self.epochs):
                ...
                if self.domains is None:
                    ...
                    domain_loss = loss_function(domain_pred, self.domains)
                    preds = self.model.model.dc(z)
                loss_vargg = self.model.vargg_loss(
                                decoded = de_feat, 
                                x = self.data, 
                                preds = preds, 
                                labels = self.adj_label, 
                ...
                if self.domains is None:
                    loss = self.kl_weight * loss_kl + loss_vargg
                else:
                    loss = self.kl_weight * loss_kl + loss_vargg + domain_loss
                loss.backward()
