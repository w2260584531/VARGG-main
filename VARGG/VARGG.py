...
from morphology import image_feature, image_crop
from adj import graph, combine_graph_dict
from model import VARGG_model
from train_func import train
from data_aug import augment_adata
...
class running():
	...
	def _fit(
		...
		vargg_training.fit() 
		vargg_embed, _ = vargg_training.process()  #sgc problems maybe
		print("Step 3: VARGG training has been Done!")
	...
	def _get_cluster_data(
		...
		sc.pp.neighbors(adata, use_rep='VARGG_embed')
		if priori:
			res = self._priori_cluster(adata, n_domains = n_domains)
		else:
			res = self._optimize_cluster(adata)
		sc.tl.leiden(adata, key_added="VARGG_domain", resolution=res)
		...
		refined_pred= refine(sample_id=adata.obs.index.tolist(), 
							 pred=adata.obs["VARGG_domain"].tolist(), dis=adj_2d, shape="hexagon")
		adata.obs["VARGG_refine_domain"]= refined_pred
		return adata
